...
@container.autowire
async def create_report(report_data: ReportCreate, report_service: Annotated[ReportService, Inject()]):
    return await report_service.create(report_data)


@router.post("/{report_id}/review", response_model=ReportRead, status_code=status.HTTP_200_OK)
@container.autowire
...
@router.get("/not-reviewed", response_model=List[ReportRead], status_code=status.HTTP_200_OK)
@container.autowire
...
@router.get("/{report_id}", response_model=ReportRead, status_code=status.HTTP_200_OK)
@container.autowire
...
@router.get("/user/{user_id}", response_model=List[ReportRead], status_code=status.HTTP_200_OK)
@container.autowire
...
@router.put("/{report_id}", response_model=ReportRead, status_code=status.HTTP_200_OK)
@container.autowire
async def update_report(report_id: UUID, report_data: ReportUpdate, report_service: Annotated[ReportService, Inject()]):
